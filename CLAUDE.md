...
**Biomes are data, not code forks.** The 3 floors (Verdant Town / Frostbound / Storm Peaks) are configs in `world/biomes.js` (`getBiome`), selected via `?level=N` on the loading screen (reload, no scene teardown); `floor.js`, `town.js` and `main.js` all read the picked biome to parameterize terrain, lighting, fog, settlement/ruin layout, enemy tint, and named places. Weather (snow / rain+lightning / wind — a per-type table in the file) lives in `world/weather.js` and is driven by real `dt`, not `sdt`. Floor 2/3 ruins load Kenney Castle Kit GLBs from `public/assets/castle/`.
