...
**Cel shading is centralized — never `new THREE.MeshLambertMaterial` again.** `src/world/toon.js` owns the cozy look: one shared 4-band gradient ramp feeding `MeshToonMaterial`. Use `toonMat(params)` as a drop-in for the old Lambert constructor on gray-box geometry, and call `toonify(mat)` / `toonifyObject(root)` on every loaded GLB (buildings, knight, golem, boss, NPCs) so imported Standard materials match. `toonify` preserves `color`, base `map`, `emissive`, and material **name** (hit-flash and the `CAO_GolemEye`/`CAO_BossEye`/`CAO_EdgeGlow` couplings depend on the name surviving). The warm grade is `renderer.toneMapping = NeutralToneMapping` (exposure 1.08) + a small warm `AmbientLight`, both in `main.js` and global so per-biome moods stay intact. Unlit FX (portal rings, hp bars, sky dome, water, blade trail) stay `MeshBasicMaterial` on purpose.
